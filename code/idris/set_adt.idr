...
list_rmDuplicates: (eq a) => list a -> list a
list_rmDuplicates nil = nil
list_rmDuplicates (h :: t) = ite (member h t)
                                  (list_rmDuplicates t)
                                  (h :: list_rmDuplicates t)

set_rmDuplicates: (eq a) => set a -> set a
set_rmDuplicates (mkSet a) = mkSet (list_rmDuplicates a)


list_combine: (eq a) => list a -> list a -> list a
list_combine nil l2 = list_rmDuplicates l2
list_combine (h1 :: t1) l2 = ite (member h1 l2)
                                    (list_combine t1 l2)
                                    (h1 :: list_combine t1 l2)

set_union (mkSet l1) (mkSet l2) = mkSet (list_combine l1 l2)
...
intersect_1: (eq a) => list a -> list a -> list a
intersect_1 nil _ = nil
intersect_1 (h::t) l2 = ite (member h l2)
                            (h :: intersect_1 t l2)
                            (intersect_1 t l2)

intersect: (eq a) => list a -> list a -> list a
intersect l1 l2 = list_rmDuplicates (intersect_1 l1 l2)

-- Return the intersection of s1 and s2
-- set_intersection: (eq a) => list a -> list a -> set a
...
set_forall p (mkSet l) = eql (list_rmDuplicates (map p l)) (true :: nil)
...
-- Return true if p is true for some v in s, otherwise else false
-- set_exists: (p: a -> bool) -> (s: set a) -> bool
set_exists p (mkSet l) = or
                            (eql (list_rmDuplicates (map p l))
                                  (true ::false :: nil))
                            (eql (list_rmDuplicates (map p l))
                                  (false :: true :: nil))
...
--pairelt takes an elt h and a listdfdfa b and returns list of all pairs
